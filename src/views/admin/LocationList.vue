<template>
  <CRow>
    <CCol sm="12">
      <CCard>
        <CCardHeader> <strong> Location </strong> List </CCardHeader>
        <CCardBody>
          <CDataTable
            :items="items"
            :fields="fields"
              ...
              >
                <CCardBody>
                  <CMedia :aside-image-props="{ height: 102 }">
                    ...
                    <p class="text-muted">Location Type: {{ item.type }}</p>
                    <p class="text-muted">Org. Code: {{ item.organization_code }}</p>
                    <p class="text-muted">Org. Name: {{ item.organization_name }}</p>
...
const fields = [
  ...
  { key: "code", _style: "min-width:50px;" },
  { key: "name", _style: "min-width:50px;" },
  { key: "type", _style: "min-width:50px;" },
  { key: "organization_name", _style: "min-width:100px;" },
  {
    key: "show_details",
    label: "",
    _style: "width:1%",
...
export default {
  ...
  methods: {
    ...
    onEdit(item){
      var self = this;
      self.$router.push({
        path: `/admin/location/${item.id}`
      });
    },
    onDeleteConfirmation(status, evt, accept) {
      var self = this;
      if (accept) {
        this.api.deleteLocation(self.itemToDelete.id).then((response) => {
          self.refreshTable();
        });
      }
      self.itemToDelete = {};
